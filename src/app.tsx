...
class TrackStatsManager {
  ...
  private setupEventListeners() {
    ...
    // Бинд SHIFT+ALT+X для добавления прослушиваний
    window.addEventListener('keydown', (e) => {
      if (e.shiftKey && e.altKey && e.key.toLowerCase() === 'x') {
        e.preventDefault();
        this.showAddPlaysModal();
      }
    });
  }
  ...
  private onTrackChange() {
    ...
    if (data?.item) {
      ...
      // Засчитываем прослушивание только если прошло больше 30 секунд с последней сессии
      if (!stats.lastPlaySession || (now - stats.lastPlaySession) > 30000) {
        stats.playCount++;
        stats.lastPlaySession = now;
  ...
  private showAddPlaysModal() {
    if (!this.currentTrack) {
      if (Spicetify?.showNotification) {
        Spicetify.showNotification('No track is currently playing', true, 2000);
      }
      return;
    }
    ...
    const modal = document.createElement('div');
    modal.className = 'add-plays-modal';
    modal.style.cssText = `
      position: fixed;
      top: 50%;
      left: 50%;
    ...
    header.style.cssText = `
      ...
      border-bottom: 1px solid #333;
    `;
    header.textContent = '➕ Add Plays Based on Track Duration';
    ...
    content.innerHTML = `
      <div style="margin-bottom: 15px;">
        <div style="font-weight: bold; margin-bottom: 5px;">Current track:</div>
        <div style="color: #1e3a8a;">${stats.title}</div>
        <div style="font-size: 12px; color: #999;">by ${stats.artist}</div>
      </div>
      <div style="margin-bottom: 15px;">
        <div style="font-weight: bold; margin-bottom: 5px;">Current stats:</div>
        <div>${stats.playCount}x • ${this.formatTime(stats.totalTime)}</div>
      </div>
      <div style="margin-bottom: 15px;">
        <label style="display: block; margin-bottom: 5px; font-weight: bold;">Track duration (mm:ss):</label>
        <input type="text" id="track-duration-input" placeholder="1:23" style="
          width: 100%;
          padding: 8px;
          background: #2a2a2a;
        ...
        <div style="font-size: 12px; color: #999; margin-top: 5px;">
          Enter track duration to calculate how many plays fit into total listening time
        </div>
      </div>
    ...
    const input = document.getElementById('track-duration-input') as HTMLInputElement;
    ...
    addBtn.addEventListener('click', () => {
      ...
      if (!match) {
        if (Spicetify?.showNotification) {
          Spicetify.showNotification('Invalid format. Use mm:ss (e.g., 1:23)', true, 2000);
        }
        return;
      }
      ...
      if (trackDurationMs <= 0) {
        if (Spicetify?.showNotification) {
          Spicetify.showNotification('Duration must be greater than 0', true, 2000);
        }
        return;
      }

      const additionalPlays = Math.floor(stats.totalTime / trackDurationMs);
      
      if (additionalPlays <= 0) {
        if (Spicetify?.showNotification) {
          Spicetify.showNotification('Not enough listening time to add plays', true, 2000);
        }
        return;
      }

      stats.playCount += additionalPlays;
      ...
      stats.dailyStats[today].playCount += additionalPlays;

      this.saveStats();
      this.updateUI();

      if (Spicetify?.showNotification) {
        Spicetify.showNotification(`Added ${additionalPlays} plays to ${stats.title}`, false, 3000);
      }
  ...
  private showTrackRanking(currentStats: TrackStats, targetElement: HTMLElement) {
    ...
    header.innerHTML = `
      ...
      <div style="display: flex; gap: 10px; align-items: center;">
        <button class="clear-stats-btn" style="
          background: #dc2626; 
          border: none; 
          color: white; 
          cursor: pointer; 
    ...
    const clearBtn = header.querySelector('.clear-stats-btn') as HTMLButtonElement;
    ...
    clearBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      if (confirm('Are you sure you want to clear all track statistics? This action cannot be undone.')) {
        this.clearAllStats();
        modal.remove();
      }
    });
